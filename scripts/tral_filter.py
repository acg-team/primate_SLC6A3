...
def cla_parser():
    ...
    parser.add_argument(
        "--input", "-i", type=str, required=True, help="Directory containing pickled RepeatLists to filter"
    )
    parser.add_argument(
        "--output", "-o", type=str, required=True, help="Directory where filtered RepeatLists will be stored"
    )
    parser.add_argument(
...
def trim_rescore(repeat, model, to_trim):
    if not to_trim in {"first", "last"}:
        raise ValueError("Can only accept 'first' or 'last' position to trim")
